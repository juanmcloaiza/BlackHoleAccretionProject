...
r_hole = 0.03
r_acc = 1e-3
x = np.linspace(r_acc,r_bulge,1025)
for col, mbh in zip(colors,Mbh):
	y = mass_inside(x,mbh,M_core,M_bulge,r_core,r_bulge)
	label_this="${\\rm M_{BH}} =$ "+str(mbh)
	imgplot = pl.semilogy(x,y, col, linewidth = 0.5*FontSize, label = label_this)
pl.axvspan(r_hole, r_shell, facecolor='g', alpha=0.5)

pl.legend(loc=4)
pl.xlabel("${\\rm r}$",fontsize=30)
pl.ylabel("$M(r)$",fontsize=30)
pl.xlim([r_acc,r_shell])
pl.ylim([1e-4,10])
#pl.title("Circularization radius as function of black hole mass")
figfile = "mass_distribution.png"
pl.savefig(figfile)
