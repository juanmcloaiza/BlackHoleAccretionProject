...
def prepare_figure():
	...
	pl.xlabel('$r_{\\rm acc}$')
	pl.ylabel('$M_{\\rm acc}/M_{\\rm acc}(N_{\\rm res} = 100 {\\rm K})$')
	pl.xlim([5e-5,7e-3])
	pl.ylim([0.5,7e2])
...
def trace_the_line(X,Y,col):
	pl.loglog( X, Y/Y[0], col+'o--', label = "$N_{\\rm res} ="+"{:.1e}".format(Nres)+"$" )
	pl.legend(loc=2)

def trace_theoretical(X):
	x = np.array([1e-4,1e-3,5e-3])
	#slope, y0 = get_fit(X,Y)
	pl.loglog(x,(x/x[0])**(2.0),'k--')
...
#	X.append(racc)
#	Y.append(Macc)
#trace_the_line(X,Y,'k')

trace_theoretical(X)
figfile = 'plot_acc_mass_vs_racc.png'
pl.savefig(figfile)
