...
def compute_acc_rate(Npart,Time):
	A = np.zeros((len(Npart),int(len(Npart)/2)))
	delta_t = np.zeros((len(Npart),int(len(Npart)/2)))
	Amean = np.zeros(len(Npart))
	dtmean = np.zeros(len(Npart))
...
#			elif(i-delta_i < 0):
#				A[i,delta_i] = - ( Npart[i+delta_i] - Npart[0] ) / (Time[i+delta_i] - Time[0])
...
#
	...
	for i in range(len(A)):
		if( ( i - delta_i >= 0 ) and ( i + delta_i < len(A) )):
				Amean[i] = - ( Npart[i+delta_i] - Npart[i-delta_i] ) / (Time[i+delta_i] - Time[i-delta_i])
...
def add_point(datafile,style,labelswitch=True):
	...
	pl.semilogy( resol, Tot_acc_mass, style, label = "$r_{\\rm acc} ="+str(r_acc)+"$" )
	if(labelswitch):
		pl.legend()
